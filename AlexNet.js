function AlexNet() {
  ...
  let depthFn = (depth) =>
    logDepth ? Math.log(depth) * depthScale : depth * depthScale;
  let widthFn = (width) =>
    logWidth ? Math.log(width) * widthScale : width * widthScale;
  let convFn = (conv) =>
    logConvSize ? Math.log(conv) * convScale : conv * convScale;

  function wf(layer) {
    return widthFn(layer["width"]);
  }
  function hf(layer) {
    return widthFn(layer["height"]);
  }
  ...
  function redraw({
    ...
    z_offset =
      -(
        sum(architecture.map((layer) => depthFn(layer["depth"]))) +
        betweenLayers * (architecture.length - 1)
      ) / 3;
    layer_offsets = pairWise(architecture).reduce(
      (offsets, layers) =>
        offsets.concat([
          offsets.last() +
            depthFn(layers[0]["depth"]) / 2 +
            betweenLayers +
            depthFn(layers[1]["depth"]) / 2,
        ]),
      [z_offset]
    );
    layer_offsets = layer_offsets.concat(
      architecture2.reduce(
        (offsets, layer) =>
          offsets.concat([offsets.last() + widthFn(2) + betweenLayers]),
        [
          layer_offsets.last() +
            depthFn(architecture.last()["depth"]) / 2 +
            betweenLayers +
            widthFn(2),
        ]
      )
    ...
    architecture.forEach(function (layer, index) {
      // Layer
      layer_geometry = new THREE.BoxGeometry(
        wf(layer),
        hf(layer),
        depthFn(layer["depth"])
      );
      ...
      layer_edges_object = new THREE.LineSegments(
        layer_edges_geometry,
        line_material
      );
      ...
      if (index < architecture.length - 1) {
        // Conv
        conv_geometry = new THREE.BoxGeometry(
          convFn(layer["filterWidth"]),
          convFn(layer["filterHeight"]),
          depthFn(layer["depth"])
        );
        conv_object = new THREE.Mesh(conv_geometry, conv_material);
        conv_object.position.set(
          layer["rel_x"] * wf(layer),
          layer["rel_y"] * hf(layer),
          layer_offsets[index]
        );
        convs.add(conv_object);

        conv_edges_geometry = new THREE.EdgesGeometry(conv_geometry);
        conv_edges_object = new THREE.LineSegments(
          conv_edges_geometry,
          line_material
        );
        conv_edges_object.position.set(
          layer["rel_x"] * wf(layer),
          layer["rel_y"] * hf(layer),
          layer_offsets[index]
        );
        ...
        summit_z =
          layer_offsets[index] + depthFn(layer["depth"]) / 2 + betweenLayers;
        next_layer_wh = widthFn(architecture[index + 1]["width"]);

        pyramid_geometry.vertices = [
          new THREE.Vector3(
            layer["rel_x"] * wf(layer) + convFn(layer["filterWidth"]) / 2,
            layer["rel_y"] * hf(layer) + convFn(layer["filterHeight"]) / 2,
            base_z
          ), // base
          new THREE.Vector3(
            layer["rel_x"] * wf(layer) + convFn(layer["filterWidth"]) / 2,
          ...
          new THREE.Vector3(
            layer["rel_x"] * next_layer_wh,
            layer["rel_y"] * next_layer_wh,
            summit_z
          ), // summit
        ];
        ...
        pyramid_edges_object = new THREE.LineSegments(
          pyramid_edges_geometry,
          line_material
        );
      ...
      if (showDims) {
        ...
        sprite.position
          .copy(layer_object.position)
          .sub(new THREE.Vector3(wf(layer) / 2 + 2, hf(layer) / 2 + 2, 0));
        sprites.add(sprite);
      ...
      if (showConvDims && index < architecture.length - 1) {
        ...
        sprite.position
          .copy(conv_object.position)
          .sub(
            new THREE.Vector3(
              convFn(layer["filterWidth"]) / 2,
              -3,
              depthFn(layer["depth"]) / 2
            )
          );
        sprites.add(sprite);
    ...
    architecture2.forEach(function (layer, index) {
      ...
      layer_object = new THREE.Mesh(layer_geometry, box_material);
      layer_object.position.set(
        0,
        0,
        layer_offsets[architecture.length + index]
      );
      layers.add(layer_object);
      ...
      layer_edges_object.position.set(
        0,
        0,
        layer_offsets[architecture.length + index]
      );
      ...
      if (showDims) {
        ...
        sprite.position
          .copy(layer_object.position)
          .sub(new THREE.Vector3(3, depthFn(layer) / 2 + 3, 3));
        sprites.add(sprite);
      }
    });
  ...
  function makeTextSprite(message, opts) {
    ...
    sprite.scale.set(10 * scale, 5 * scale, 1.0);
